...
public struct IOSCapabilityMatrix: Sendable, Codable {
    ...
    private static func probeIOS() -> IOSCapabilityMatrix {
        ...
        return IOSCapabilityMatrix(
            ...
            sandboxFS: sandboxFS,
            sandboxRoots: roots,
            fileWatching: Capability(.limited, "DispatchSource.makeFileSystemObjectSource solo para paths del sandbox; sin @parcel/watcher integral"),
            securityBookmarks: Capability(.limited, "UIDocumentPickerViewController + URL.bookmarkData + startAccessingSecurityScopedResource (requiere interacción usuario)"),
            arbitraryPaths: Capability(.unavailable, "Sandbox restringe a contenedores de la app salvo security bookmarks vía picker"),
            processExec: Capability(.unavailable, "Process/NSTask no están en el SDK iOS; sandbox prohíbe fork/execve/posix_spawn de binarios arbitrarios"),
            shell: Capability(.unavailable, "No existen /bin/bash, /bin/sh, /bin/zsh en iOS; sin TTY"),
            ptyTTY: Capability(.unavailable, "No hay API pública de PTY (openpty/posix_openpt) ni TTY crudo; @opentui/@lydell/node-pty no operables"),
            networkTLS: Capability(.available, "URLSession async/await + ATS"),
            webSocket: Capability(.available, "URLSessionWebSocketTask"),
            localServer: Capability(.available, "Network.framework NWListener en 127.0.0.1"),
            mdns: Capability(.limited, "NetService/NWBrowser existen; no equivalente exacto a bonjour-service npm"),
            sqlite: Capability(.available, "SQLite del sistema accesible vía C API / GRDB / SQLite.swift / CoreData"),
            sqliteBunFlavor: Capability(.unavailable, "@effect/sql-sqlite-bun requiere runtime Bun; no existe en iOS"),
            bunRuntime: Capability(.unavailable, "Bun publica linux/darwin/win × {x64,arm64}; no existe target Bun iOS"),
            javaScriptEngine: Capability(.limited, "JavaScriptCore + WKWebView ejecutan JS puro; sin APIs bun:/node:/node:fs/PTY"),
            treeSitterNative: Capability(.unavailable, "No hay toolchain en iOS; tree-sitter-* necesita compilar para arm64-ios"),
            posixEnv: Capability(.unavailable, "No hay $SHELL/$HOME POSIX/.zshrc/EDITOR; setenv solo dentro del proceso propio"),
            codeSigning: Capability(.limited, "Solo macOS (Xcode/codesign); en iOS unsigned IPA se construye en CI macOS"),
            keychain: Capability(.available, "Keychain Services (SecItem*) hardware-backed"),
            modelAPIRemote: Capability(.available, "URLSession HTTP/HTTPS a cualquier LLM API compatible (OpenAI-style)")
        )
    }
    #else
    private static func probeIOS() -> IOSCapabilityMatrix {
